...
from reddit import get_latest_new
from models import *
...
def get_updates(last_updated):
    ...
    response = requests.get(
        f"{API_BASE}/getUpdates", params={"offset": last_updated + 1}
    )
    ...
    if r["ok"]:
        for req in r["result"]:
            ...
            try:
                ...
                log.debug("Looks like no chat text was detected... monving on")
            ...
            if r is not None and r.group(1) == "source":
                if r.group(2):
                    ...
                    log.info(
                        f"Sources set for {person_id} to {sources_dict[person_id]}"
                    )
                    with db.atomic() as txn:
                        try:
                            sources = Source.create(
                                person_id=person_id, fetch_from=sources_dict[person_id]
                            )
                            log.debug(f"Inserted row id: {sources.person_id}")
                        except peewee.IntegrityError:
                            sources = Source.update(
                                fetch_from=sources_dict[person_id]
                    ...
                    post_message(
                        person_id,
                        "We need a comma separated list of subreddits! No subreddit, no news :-(",
                    )
...
I will have this configurable soon.
            ...
            if split_chat_text[0] == "/fetch" and (person_id not in skip_list):
                ...
                try:
                    ...
                    summarized_news = get_latest_new(sub_reddits)
